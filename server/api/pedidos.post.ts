...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const pedido = await sql`
      INSERT INTO pedidos (empresa_id, cliente_id, nome_cliente, status, valor_total)
      VALUES (${user.empresa_id}, ${body.cliente_id}, ${body.nome_cliente}, ${body.status || 'Orçamento'}, ${body.valor_total})
      RETURNING id
    `
    const pedidoId = pedido[0].id

    // 2. Insere os Itens (Loop)
