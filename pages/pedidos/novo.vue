...
async function submitQuote() {
  ...
  try {
    const payload = {
      ...
      // O campo status é crucial aqui: todo novo pedido é um ORÇAMENTO
      status: 'ORCAMENTO', 
    ...
    // A SUBMISSÃO DEVE APONTAR PARA O SEU NOVO ENDPOINT UNIFICADO (Ex: /api/pedidos)
    ...
    if (response.success) {
      alert(`✅ Pedido #${response.quoteId} salvo como Orçamento!`);
      // Redireciona para a lista unificada de pedidos
      router.push('/pedidos'); 
    }

  } catch (error) {
    console.error('Erro:', error);
    alert('Erro ao salvar o pedido. Verifique os logs do servidor.');
  } finally {
